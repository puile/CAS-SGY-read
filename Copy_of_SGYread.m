...
for i = 1:length(sgy_files)
    ...
    try
        data = readsegyfile(fullfile(input_folder, filename));
    ...
    % 其他处理步骤...
        % **数据归一化到 int16 范围**
    max_val = max(abs(data(:))); % 找到数据中的最大绝对值
    if max_val > 0
        normalized_data = data / max_val; % 归一化到 [-1, 1] 范围
    else
        normalized_data = data; % 如果数据全是0，则不进行归一化
    end
    int16_data = int16(normalized_data * 32767); % 缩放到 int16 范围并转换为 int16 类型
    ...
    name = sprintf('20250310_L%03d', file_counter); % 生成如 20250310_L001 格式的文件名
    output_filename_rad = fullfile(output_folder,  [name, '.RAD']);
    output_filename_rd3 = fullfile(output_folder, [name, '.RD3']);
