...
# 根目录路径
root_folder = r'E:\02processingdata\01ftq\mtl\ASCII\images'

# 遍历根目录下的所有子文件夹
for subdir, _, _ in os.walk(root_folder):
    # 构建输入和输出文件夹路径
    input_folder = os.path.join(subdir, "result")
    ...
    # 获取所有 .png 图像文件并排序
    image_files = sorted([f for f in os.listdir(input_folder) if f.endswith('.png') and f != "imgsave_with_coords.png"])
    L = 20
    overlap_ratio = 3 / L

    # 确保有足够的图像文件进行拼接
    if len(image_files) < 2:
        print(f"图像文件不足以进行拼接，跳过文件夹: {input_folder}")
        continue
    ...
    # 添加坐标轴标签
    cv2.putText(canvas, "Distance (m)", (canvas_width // 2, canvas_height-40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
